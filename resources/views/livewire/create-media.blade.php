        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <form>
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mb-4">
                        <label for="media_type_id" class="block text-gray-700 text-sm font-bold mb-2">Type:</label>
                        <select wire:model="media_type_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="media_type_id">
                            <option value="">Select Type</option>
                            @foreach($mediaTypes as $mediaType)
                                <option value="{{ $mediaType->id }}">{{ $mediaType->type }}</option>
                            @endforeach
                        </select>
                        @error('media_type_id') <span class="text-red-500">{{ $message }}</span>@enderror
                    </div>
                </div>
